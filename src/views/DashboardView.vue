...
<template>
  <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-if="loading" class="card p-10 text-center text-slate-500">Carregando…</div>

    <template v-else>
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="card p-4">
          <p class="text-xs uppercase tracking-wide text-slate-500">Total alocado</p>
          <p class="text-xl font-semibold tabular-nums mt-1">{{ fmtMoney(kpi.alocado) }}</p>
        </div>
        <div class="card p-4">
          <p class="text-xs uppercase tracking-wide text-slate-500">Utilizado</p>
        ...
        <div class="card p-4">
          ...
          <p class="text-xl font-semibold tabular-nums mt-1" :class="kpi.saldo < 0 ? 'text-red-600' : 'text-green-700'">
            {{ fmtMoney(kpi.saldo) }}
          </p>
        </div>
        <div class="card p-4">
          <p class="text-xs uppercase tracking-wide text-slate-500">NFs lançadas</p>
      ...
      <div class="card overflow-x-auto">
        <div class="px-4 pt-4 pb-2">
          <h2 class="font-medium text-slate-700">Execução por grupo</h2>
        </div>
        <table class="w-full text-sm min-w-[56rem]">
          ...
          <tbody class="divide-y divide-slate-200">
            <tr v-for="g in resumo" :key="g.grupo_id" class="hover:bg-slate-50">
              <td class="px-4 py-2.5">
                <span class="font-medium">{{ g.nome }}</span>
                <span
                  v-if="g.status !== 'vigente'"
                  class="ml-2 text-xs text-slate-500 capitalize"
                >({{ g.status }})</span>
              </td>
              <td class="px-4 py-2.5">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs tabular-nums text-slate-600 w-12 text-right">
                    {{ pctUso(g).toFixed(1) }}%
                  </span>
                </div>
              </td>
              ...
              <td
                ...
                :class="Number(g.saldo) < 0 ? 'text-red-600' : ''"
              >{{ fmtMoney(g.saldo) }}</td>
